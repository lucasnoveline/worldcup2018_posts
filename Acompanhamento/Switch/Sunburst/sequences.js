var file_path = document.getElementById("sequence").className;

d3.text(file_path, function(text) {
  ...
  {
    c = csv[i][0].split('-')[0];
    if (!order_vector.includes(c))
    {
      order_vector.push(c);
    }
  }
  order_vector.push('Oitavas');
  order_vector.push('Quartas');
  order_vector.push('Semi');
  order_vector.push('Final');
  order_vector.push('Campeão');
  order_vector.push('Eliminado');
